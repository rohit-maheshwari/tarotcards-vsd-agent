...
router.put('/user/update', async (req, res) => {
    ...
    try {
        const { personId, firstName, lastName, emailAddress, classStanding, graduationDate } = req.body;
        const person = await Person.findOne({ personId: personId });
        if (person) {
            const editedPerson = {
            ...
            const result = await Person.findOneAndUpdate({ personId: personId }, { $set: editedPerson }, { new: true });
            if (result) {
                res.status(200).json({
